...
class ImageInstance:
    ...
    def init_reader(self, input_path, series_uid=None, read_meta=True):
        ...
        if os.path.isdir(input_path):
            ...
            # get_series_with_most_slices returns (series_uid, directory) tuple
            if isinstance(series_uid, tuple):
                series_id, series_dir = series_uid
    ...
    def get_series_with_most_slices(input_path):
        ...
        # First, check if there are DICOM files directly in the input_path
        direct_series_IDs = sitk.ImageSeriesReader.GetGDCMSeriesIDs(input_path)

        if direct_series_IDs:
            # No nesting - DICOM files are directly in the folder
            for series_ID in direct_series_IDs:
                series_filenames[series_ID] = (
                    input_path,
                    len(
                        sitk.ImageSeriesReader.GetGDCMSeriesFileNames(
                            input_path, series_ID
                        )
                    ),
                )
            ...
            # Return just the series ID for non-nested case
        ...
        # Return both series ID and the directory where it was found for nested case
        proper_series_dir = series_filenames[proper_series_ID][0]
        return (proper_series_ID, proper_series_dir)
